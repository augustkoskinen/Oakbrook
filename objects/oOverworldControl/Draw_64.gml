if(global.overworldaction != -1&&startaction) {
	switch(global.overworldaction) {
		...
		case 5: {
			...
			for(var i = 0; i < array_length(oCardControl.cards); i++)
				if(array_get(oCardControl.cards,i).index==16||array_get(oCardControl.cards,i).index==17)
					num++;
		...
		switch(global.overworldaction) {
			...
			case 5: {
				for(var i = 0; i < 2; i++) {
					...
					if(i==0&&oCardControl.vessels>=3) canchoose = false;
					if(i==1&&oCardControl.idols>=3) canchoose = false;
					...
					&&canchoose) {
						...
						if(global.mousedown) {
							...
							if(i==0) {
								oCardControl.vessels++;
								oCardControl.handmax++;
							} else if(i==1) {
								oCardControl.idols++;
								oCardControl.coinmax++;
							}
				...
				for(var i = array_length(selectcards) - 1; i >= 0; i--) {
					...
					if(global.mousedown&&hoverid==noone) {
						trashselect = array_create(0);
						global.mousedown = false;
					} else if(hoverid==card.id||selected) {
						shader_set(shWhiteOutline)
						var texelW = texture_get_texel_width(sprite_get_texture(sCard,card.index))
						var texelH = texture_get_texel_height(sprite_get_texture(sCard,card.index))
						shader_set_uniform_f(pixelDims,texelW,texelH)
